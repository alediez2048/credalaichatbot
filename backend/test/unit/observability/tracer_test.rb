...
module Observability
  class TracerTest < ActiveSupport::TestCase
    test "enabled? is false when LANGFUSE_SECRET_KEY is blank" do
      with_env "LANGFUSE_SECRET_KEY" => nil do
        assert_not Tracer.enabled?
      end
    end

    test "enabled? is true when LANGFUSE_SECRET_KEY is set" do
      with_env "LANGFUSE_SECRET_KEY" => "sk-test" do
